...
Phase 2 shipped a **pilot** MCP (`kingdom-ops`) and the OS contracts (sync → Throne, wiki tools, judge). The remaining work is **fleet connectivity**: every active venture with a real repo root is reachable from Cursor/agents via the same control surface (registry → sync/Throne + MCP + skills), without widening the secret surface.
