...
Decision note: **do not adopt LangChain/LangGraph as a Kingdom dependency by default.** Prefer custom orchestration + FSM/eval/cost/provenance discipline already in the fleet. Keep framework literacy for interviews and selective borrowing of patterns.
...
| **Kingdom default** | Custom orchestration + harness FSM/KG + phase gates. Claim language: “built custom orchestration; can discuss LangGraph/MCP tradeoffs” ([[agentic-resume-gates]]). |

Kingdom already maps closer to **LangGraph concepts** than LangChain: WhatsApp qualify→booking FSM, Comic/City tool loops, YouTube multi-agent desk, brain harness control FSM ([[brain-harness-fsm]]).

## What the roles actually do

**AI / Applied AI engineer** — productize models: APIs, queues, caching, cost/latency, fallbacks; RAG vs memory vs fine-tune; eval harnesses in CI; traces + failure taxonomy + usage metering.

**Agentic engineer** — tool schemas + loops (ReAct vs planner); when FSM beats free-form agent; multi-agent handoff / ownership / race control; human handoff + audit; framework literacy without worship.

**FDE (Forward Deployed Engineer)** — sit in the customer workflow; thinnest agent that changes a real process; integration over clever prompting; pilot metrics and field failure modes. WhatsApp Phase 8+ unlocks FDE-strength claims ([[agentic-resume-gates]], [[ventures/whatsapp-voice]]).

## Patterns to borrow (without new frameworks)

1. **Explicit workflow graph** — states, legal transitions, human interrupt points; free-form LLM only inside bounded nodes.
...
4. **Eval / lint gate before “done”** — goldens, VLM/rubric gates (Comic pattern).
...
8. **One control plane** — `npm run sync` → Throne; onboard via [[onboard-new-project]].
...
Same shape FDE + agentic product teams use:
...
- Keep drill list in [[agentic-interview-prep]] (tool-calling backend, long-running jobs, cost caps, RAG vs memory, eval CI, multi-agent desk, WhatsApp FSM).
...
| Scaled AI product (pre-pilot) | Lab systems with spend ceilings + usage accounting |
