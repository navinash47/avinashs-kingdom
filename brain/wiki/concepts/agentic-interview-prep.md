...
Interview stack for Applied Artificial Intelligence / Agent Engineer roles (not classical FAANG-Software Development Engineer-first). Pair with [[ventures/job-jugaad]] and Track A resume.

**Mix:** Coding 40% · Artificial Intelligence system design 30% · Agentic depth 25% · Light classical Software Development Engineer 5%.
...
## 2. System design for Artificial Intelligence products

One drill per week. Outline: requirements to API to data to queues/workers to models/tools to evals to cost/latency to failure modes.
...
3. Design **multi-tenant Large Language Model cost caps** (per-user ceilings, fallback models, caching).
4. Design **RAG vs agent memory** for a support bot; when each wins.
5. Design **eval regression in CI** (golden sets, Large Language Model-as-judge pitfalls, flaky gates).
6. Design a **multi-agent YouTube desk** (work items, provenance lint, cut control).
7. Design **WhatsApp lead-qualify Finite State Machine** with human handoff and audit trail.
...
| State machines vs planners | When Finite State Machine beats free-form agent | WhatsApp qualify to booking Finite State Machine |
| Multi-agent handoff | Work items, ownership, race control | YouTube multi-agent desk |
| Evals | Golden sets, Large Language Model-as-judge failure modes | BioNLP R@10; Comic Vision Language Model rubric |
...
**Ship one public artifact:** short eval report from Comic or YouTube (metrics + failure cases) > another DeepLearning.Artificial Intelligence cert.

## 4. Light classical Software Development Engineer (5%)

Keep Mastercard stories ready: latency, PCI tokenization, Java 8 to 17, Jenkins, Azure monitoring (−25% manual). Enough for legacy screens; do **not** re-specialize Java/Software Development Engineer.
...
| Sat | 1 Artificial Intelligence system-design drill (45–60 min writeup) |
