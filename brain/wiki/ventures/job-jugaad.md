...
Applications tracker: **company + specific role** through a pipeline (wishlist to applied to interviewing to offer / rejected / ghosted). File-backed at `data/applications.json`. Auto-updates `STATUS.md` progress as `applied-or-beyond / target` (default target 30).
...
Config: `~/Projects/job-jugaad/config/resume-tracks.json` · mirrored in `data/applications.json` `primary_tracks`. Software Development Engineer Entry excluded (new-grad only).
