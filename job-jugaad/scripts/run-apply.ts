...
async function main() {
  ...
  const preferred = readJson<QueueItem[]>('data/apply-targets.json', [])
  if (!queue.length) {
  ...
  console.log(
    'Job Jugaad apply — Gmail prompted only if OTP/verify needed (memory-only).',
  )

  const fromTargets = preferred.filter((p) =>
    queue.some((q) => q.id === p.id && (q.status === 'queued' || q.status === 'waiting-on-you')),
  )
  const targets = (fromTargets.length ? fromTargets : queue.filter(
    (q) => q.status === 'queued' || q.status === 'waiting-on-you',
  )).slice(0, limit)
  ...
  for (const item of targets) {
    ...
    const idx0 = queue.findIndex((q) => q.id === item.id)
    if (idx0 >= 0) {
      queue[idx0].status = 'filling'
      queue[idx0].updatedAt = new Date().toISOString()
    ...
    if (idx >= 0) {
      queue[idx] = {
        ...
        error: result.error,
        learnedQuestions: result.learnedQuestions,
          ...
          await appendGapsExcel([
            {
              ...
              why: result.error,
              learnNext:
                'Inspect failure; fix profile facts or try again after CAPTCHA',
            },
          ])
        }
      }
